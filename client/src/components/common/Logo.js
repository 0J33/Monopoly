...
// The app's brand mark: two dice on the accent→success gradient tile, drawn
// dark like the pips of the die face it replaced. The same drawing as
// /favicon-v4.svg and the home-screen icons, so the header and the tab match.

export default function Logo({ size = 32, style }) {
    const grad = `logoGrad_${size}`;
    return (
        <svg viewBox="0 0 24 24" width={size} height={size} xmlns="http://www.w3.org/2000/svg" style={style} aria-label="Monopoly">
            <defs>
            ...
            <rect width="24" height="24" rx="5.5" fill={`url(#${grad})`} />
            <g fill="#0b0f17" transform="translate(4.2500,4.2500) scale(0.64583)">
